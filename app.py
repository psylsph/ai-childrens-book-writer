...
if use_existing_raw:
    ...
    with open(book_name_raw, "r", encoding='utf-8') as book:
        writer_result = book.read()
        raw_chapters = writer_result.split(r"**Chapter ")
        if len(raw_chapters) < num_chapters:
            raw_chapters = writer_result.split(r"Chapter ")
            if len(raw_chapters) < num_chapters:
                raise ValueError("Not enough chapters in the outline")

        print(f"Found {len(raw_chapters)-1} chapters in the outline\n")
    ...
    writer = Agent(  
        ...
        system_prompt=(f"""Prompt:
                ...
                6. Word Count Requirement: Each chapter MUST be at least 3000 words (30000 characters) and MUST not exceed 400 words. This is a hard requirement. Do not end the chapter until this requirement is met.
    ...
    with open(book_name_raw, "w", encoding='utf-8') as book:
        try:
            ...
            for chapter_num in range(1, num_chapters+1):
                ...
                book.write(f"Chapter {chapter_num}:\n")
...
human_editor = Agent(  
    ...
    system_prompt=(f"""Prompt:
            ...
            Just provide the updated chapter text with no summary or narrative."""),
    retries=5)
